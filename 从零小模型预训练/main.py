...
# 初始化模型与优化器
# -----------------------------
model = ToyGPT(vocab_size, d_model, n_layer, n_head, d_ff, block_size).to(device)
...
# -----------------------------
model.eval()
context = torch.tensor([stoi[c] for c in "韩立是谁"], dtype=torch.long).unsqueeze(0).to(device)
with torch.no_grad():
    for _ in range(50):
